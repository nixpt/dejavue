...
- ✅ **`domain_owner` field + recall-by-owner** *(shipped, unreleased)* — promote from the buried Tier-4 "ownership maps" row to a first-class additive event field via `--domain-owner` plus `dejavue owners [NAME]`, mirroring `entities[]`. (scratch: `deja1.md`)
